...
function MediaDetails() {
  ...
  const handleReviewUpsert = async (newStatus, newRating, newComment) => {
    try {
      await triggerUpsertReviews({
        ...
        comment:
          newComment !== undefined ? newComment : myReview?.comment || null,
      }).unwrap();
    } catch (err) {
  ...
  if (isItemLoading)
    return (
      <div className="bg-slate-950 text-white min-h-screen flex items-center justify-center font-bold italic tracking-widest">
        Загрузка...
      </div>
    );
    ...
    <div className="bg-slate-950 text-slate-200 min-h-screen w-full relative pb-20">
      <div className="fixed inset-0 z-0">
        <img
          ...
          className="w-full h-full object-cover opacity-30 blur-2xl scale-105"
          alt=""
        />
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 pt-8">
        <button
          onClick={() => navigate("/")}
          className="flex items-center gap-2 text-slate-500 hover:text-indigo-400 mb-6 transition-colors"
        >
          ← Назад в библиотеку
        </button>
            ...
            <div className="bg-slate-900/60 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl p-6 md:p-10 mb-14">
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                <div className="md:col-span-4 lg:col-span-3">
                  <img
                    src={itemData.poster_url}
                    className="w-full rounded-2xl shadow-2xl border border-white/5 object-cover aspect-2/3"
                    alt=""
                  />
                </div>
                ...
                <div className="md:col-span-8 lg:col-span-9 flex flex-col">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6 text-[10px] uppercase font-bold text-slate-500">
                    {itemData.meta_data?.genres?.map((g, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 bg-slate-800/50 rounded border border-white/5"
                      >
                        {g.genre}
                      </span>
                    ))}
                  ...
                  <div className="space-y-4 mb-8">
                    ...
                    <p className="text-slate-300 leading-relaxed text-base">
                      {itemData.description || "Описания нет..."}
                    </p>
                    <div className="text-sm text-slate-500 italic">
                      Добавлено:{" "}
                      <span className="text-slate-300 font-bold not-italic">
                        {itemData.profiles?.name}
                      </span>
                    </div>
                  </div>
                  {!user.is_anonymous && (
                        ...
                        <div className="flex items-center gap-4 pb-1">
                          <StatusComponent
                            ...
                            handleStatusChange={(s) =>
                              handleReviewUpsert(s, undefined, undefined)
                            }
                          ...
                          <RatingComponent
                            ...
                            handleRatingChange={(r) =>
                              handleReviewUpsert(undefined, r, undefined)
                            }
                          />
                        </div>
                      </div>
