...
import { selectUserId, selectCurrentUser } from "../features/auth/authSlice";
...
function MediaItem({ item, setItemToEdit, setIsModalOpen }) {
  ...
  const parsedGenres = meta_data?.genres
    ?.map((genresItem) => genresItem.genre)
    .slice(0, 2)
    .join(" • ");
  ...
  const author = item.profiles?.name || "некто";
    ...
    >
      {isOwner && (
        <>
          <button
            ...
            className="
              absolute top-1 right-1 z-30 p-1 rounded-full bg-black/70  text-white backdrop-blur-sm transition-all
              opacity-100 lg:opacity-0 lg:group-hover:opacity-100 data-[open=true]:opacity-100"
          >
          ...
          {isMenuOpen && (
            <>
              ...
              <div
                className="
                  absolute top-10 right-2 z-50 flex flex-col gap-1 p-1 min-w-35
                bg-black/70 backdrop-blur-md border border-white/10 
                  rounded-xl shadow-2xl  animate-in fade-in zoom-in-95 duration-100 origin-top-right"
              >
                <button
                  onClick={(e) => {
                    e.stopPropagation();
      ...
      <div className="relative w-28 shrink-0 lg:h-60 lg:w-full overflow-hidden">
        ...
        <div className="absolute bottom-2 left-2 bg-black/70 backdrop-blur-md text-white text-sm sm:text-xs font-bold px-2 py-1 rounded">
          {CurrentCategory && <span>{CurrentCategory.text}</span>}
        </div>
      </div>
      ...
      <div className="flex flex-1  flex-col p-2 md:p-4">
        ...
        <p className="text-slate-400 lg:text-base text-sm uppercase">
          {parsedGenres}
        </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-slate-900 rounded-2xl border border-slate-800 p-6 max-w-sm w-full text-center">
            ...
            <p className="text-slate-400 mb-6">
              Вы точно хотите удалить запись "{item.title}"?
            </p>
            <button
              onClick={(e) => {
                e.stopPropagation();
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                deleteMedia(item.id).then(() => setIsDeleteModalOpen(false));
              }}
